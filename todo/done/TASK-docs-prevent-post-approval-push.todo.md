...
- [ ] Все распределяемые `.md` в `docs/git-workflow/` помечены YAML front matter `package: prikotov/git-workflow` первым блоком файла; шаблоны `*.template.md` (`templates/`) не помечаются — их заполняет проект-потребитель.
