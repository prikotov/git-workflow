...
Для подготовки файлов срочного исправления база и цель PR — выбранная ветка патч-релиза: [отдельный сценарий](release.md#hotfix-и-patch-release).
...
- тег обычного релиза отмечает выбранный коммит основной ветки;
...
- если имя ветки занято, сначала завершается предыдущая линия; принудительная перезапись запрещена.
