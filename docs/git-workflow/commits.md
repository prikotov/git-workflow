...
Черновой коммит — промежуточная фиксация многоэтапной работы. Его делает сам исполнитель: целенаправленно, на атомарном и логически завершённом этапе. Формат остаётся Conventional Commits: отдельного типа `wip` нет, черновик оформляется допустимым типом.
