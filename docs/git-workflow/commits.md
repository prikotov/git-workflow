...
Черновой коммит — промежуточная фиксация многоэтапной работы. Его делает сам исполнитель: целенаправленно, на атомарном и логически завершённом этапе. Прерванная сессия (сбой, таймаут) — не повод для чернового коммита чужой рукой: незакоммиченный diff — это состояние, на котором остановилась работа. Формат остаётся Conventional Commits: отдельного типа `wip` нет, черновик оформляется допустимым типом.
