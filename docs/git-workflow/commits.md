...
Черновой коммит — промежуточная фиксация многоэтапной работы (например, при перезапусках агента-исполнителя по таймауту). Формат остаётся Conventional Commits: отдельного типа `wip` нет, черновик оформляется допустимым типом.

- Формат: `chore(<scope>): wip <что сделано> / черновик <что сделано>`, где `<что сделано>` — короткое описание выполненного этапа. Правила subject сохраняются, кроме повелительного наклонения: его заменяет пометка `wip … / черновик …`.
- Пример: `chore(app-web): wip add buy form step / черновик шага формы покупки`.
- Не вставляй ссылки вида «PR #N:» в subject и тело — парсер трактует их как футер.
- Перед слиянием в основную ветку приведи черновые коммиты к итоговым типам (сквош или переформулировка), чтобы история и changelog остались чистыми.
